(async () => {
  ...
  // [최종 수정] 100% 검증된 유저님의 기존 21개 토큰만으로 구성!
  const tokenConfig = {
    Primary: {
      Default:  { Bg: "brand-primary",        Text: "text-inverse" },
      Hover:    { Bg: "brand-primary-hover",  Text: "text-inverse" },
      Active:   { Bg: "brand-primary-hover",  Text: "text-inverse" }, // Hover 재활용
      Disabled: { Bg: "bg-surface",           Text: "text-muted" }    // 검증된 기존 토큰 재활용!
    },
    Secondary: {
      Default:  { Bg: "bg-surface",           Text: "text-primary" },
      Hover:    { Bg: "bg-base",              Text: "text-primary" },
      Active:   { Bg: "bg-base",              Text: "text-primary" }, // Hover 재활용
      Disabled: { Bg: "bg-surface",           Text: "text-muted" }    // 검증된 기존 토큰 재활용!
    },
    Danger: {
  ...
  compSet.name = "Foundation Button (Perfect & Safe)";
  ...
  console.log("🚀 [v4.6] 100% 검증된 토큰 전용 버튼 세트 생성 완료!");
})();