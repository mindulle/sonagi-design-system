(async () => {
  ...
  async function getFoundationToken(name, fallbackRgb) {
    ...
    // [최종 병기] 억센트 토큰 강제 수입 시도
    if (name === "accent") {
      try {
        // Foundation의 accent 변수를 강제 수입 시도 (일반적인 라이브러리 연동 시 동작함)
        ...
        for (const lib of allCollections) {
          try {
            ...
            const target = vars.find(v => v.name.includes("accent"));
            if (target) {
              const importedVar = await figma.variables.importVariableByKeyAsync(target.key);
              console.log(`✨ 강제 수입 성공: 'accent'`);
              variableMap["accent"] = importedVar;
              return importedVar;
            }
          } catch(e) {}
        }
  ...
  const tokenConfig = {
    Primary: {
      ...
      Active:   { Bg: "brand-primary-hover",  Text: "text-inverse" }, 
  ...
  for (const type of types) {
    for (const state of states) {
      ...
      const bgToken = await getFoundationToken(config.Bg, rgb(219, 108, 102)); // accent fallback color
  ...
  compSet.name = "Foundation Button (Auto Import)";
  ...
  console.log("🚀 [v4.7] 강제 수입 기능이 추가된 버튼 세트 생성 완료!");
})();