async function createCheckboxComponent() {
  ...
  for (const checked of checks) {
    ...
    for (const state of states) {
      ...
      box.name = "Box"; box.resize(20, 20); box.cornerRadius = 4; box.layoutMode = "HORIZONTAL"; box.primaryAxisSizingMode = "FIXED"; box.counterAxisSizingMode = "FIXED"; box.resize(20, 20); box.primaryAxisAlignItems = "CENTER"; box.counterAxisAlignItems = "CENTER"; 
      ...
      if (checked === "True") {
        const icon = figma.createVector(); icon.name = "Icon";
        ...
        if (iconColor) { icon.strokes = [bindColor(iconColor)]; icon.strokeWeight = 2; icon.strokeCap = "ROUND"; icon.strokeJoin = "ROUND"; }
        icon.x = 0; icon.y = 0; box.appendChild(icon);
      }
  ...
  componentSet.name = "Checkbox"; componentSet.x = 100; componentSet.y = 100;
