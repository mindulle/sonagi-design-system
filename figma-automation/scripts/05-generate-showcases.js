async function generateShowcases() {
  if (figma.currentPage.selection.length !== 1 || figma.currentPage.selection[0].type !== "COMPONENT_SET") {
    console.error("❌ 마스터 컴포넌트 세트(보라색 점선 테두리)를 정확히 1개만 선택해주세요.");
    return;
  }
  ...
  if (lightMode) lightSection.setExplicitVariableModeForCollection(colorsColl, lightMode.modeId); // ID대신 객체 전달

  master.children.forEach(variant => {
    const inst = variant.createInstance();
    inst.x = lightSection.x + variant.x + 50; 
    inst.y = lightSection.y + variant.y + 50;
  ...
  if (darkMode) darkSection.setExplicitVariableModeForCollection(colorsColl, darkMode.modeId); // ID대신 객체 전달

  master.children.forEach(variant => {
    const inst = variant.createInstance();
    inst.x = darkSection.x + variant.x + 50; 
    inst.y = darkSection.y + variant.y + 50;
    darkSection.appendChild(inst);
  });

  console.log(`🎉 ${master.name}의 쇼케이스 2종이 완벽하게 생성되었습니다!`);
